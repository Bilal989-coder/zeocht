...
import { Star, MapPin, Video, Clock, Users, CalendarIcon, ArrowLeft, Heart, Share2, Grid3x3 } from "lucide-react";
...
const ExperienceDetail = () => {
  ...
  const generateTimeSlots = (startTime: string, endTime: string, durationMinutes: number) => {
    ...
    while (currentHour * 60 + currentMin + durationMinutes <= endTimeInMinutes) {
  ...
  // Fetch guide availability when date changes
  const fetchAvailability = async (selectedDate: Date, guideId: string, serviceDuration: number) => {
    ...
    try {
      ...
      if (availability && availability.length > 0) {
        ...
        // No availability set - show default slots
  ...
  const fetchServiceDetails = async () => {
    try {
      ...
      const {
        data: serviceData,
        error: serviceError
      } = await supabase.from("services").select("*").eq("id", id).eq("status", "active").single();
      ...
      if (serviceData?.guide_id) {
        const {
          data: profileData
        } = await supabase.from("profiles").select("*").eq("id", serviceData.guide_id).single();
      ...
      const {
        data: reviewsData
          ...
          *,
          explorer:explorer_id(full_name, avatar_url)
        `).eq("service_id", id).order("created_at", {
        ascending: false
      }).limit(5);
  ...
  const handleChatWithGuide = async () => {
    ...
    const {
      data,
      error
    } = await createConversation(service.guide_id, service.id);
    if (error) {
      toast({
        title: "Error",
  ...
  const displayImages = service.image_urls && service.image_urls.length > 0 ? service.image_urls : [service.image_urls?.[0] || ""];
  return <div className="min-h-screen bg-background">
      <Navbar />
        ...
        <div className="relative mb-8 rounded-xl overflow-hidden">
          {displayImages.length === 1 ? (
            ...
            <div className="grid grid-cols-2 gap-2 h-[400px] sm:h-[500px]">
              <img src={displayImages[0]} alt={service.title} className="w-full h-full object-cover" />
              <img src={displayImages[1]} alt={`${service.title} 2`} className="w-full h-full object-cover" />
            </div>
          ) : displayImages.length === 3 ? (
            // Three images - one large left, two stacked right
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 h-[400px] sm:h-[500px]">
              <img src={displayImages[0]} alt={service.title} className="w-full h-full object-cover" />
              <div className="grid grid-rows-2 gap-2">
                <img src={displayImages[1]} alt={`${service.title} 2`} className="w-full h-full object-cover" />
                <img src={displayImages[2]} alt={`${service.title} 3`} className="w-full h-full object-cover" />
              </div>
            </div>
          ) : displayImages.length === 4 ? (
            // Four images - one large left, three on right (2 top, 1 bottom)
            <div className="grid grid-cols-1 sm:grid-cols-4 grid-rows-2 gap-2 h-[400px] sm:h-[500px]">
              ...
              <div className="col-span-2 sm:col-span-2 row-span-1">
                <img src={displayImages[3]} alt={`${service.title} 4`} className="w-full h-full object-cover" />
              </div>
            </div>
          ) : (
            // Five or more images - one large left, 2x2 grid on right
            <div className="grid grid-cols-1 sm:grid-cols-4 grid-rows-2 gap-2 h-[400px] sm:h-[500px]">
              <div className="col-span-1 sm:col-span-2 row-span-2">
                <img src={displayImages[0]} alt={service.title} className="w-full h-full object-cover" />
              </div>
          ...
          {displayImages.length > 5 && (
            <Button variant="outline" size="sm" className="absolute bottom-4 right-4 bg-background gap-2 shadow-lg hover:bg-background/90">
        ...
        <div className="grid lg:grid-cols-3 gap-8 lg:gap-16">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="pb-8 border-b border-border">
              ...
              {service.whats_included && service.whats_included.length > 0 && <ul className="space-y-3">
                  {service.whats_included.map((item, index) => <li key={index} className="flex items-start gap-3">
            ...
            <div className="pb-8 border-b border-border">
              ...
              {service.requirements && service.requirements.length > 0 && <div className="mt-6">
                  <h3 className="font-semibold mb-3">Requirements</h3>
                  <ul className="space-y-2">
                    {service.requirements.map((req, index) => <li key={index} className="text-sm text-muted-foreground flex items-start gap-2">
            ...
            <div className="pb-8 border-b border-border">
              ...
              {reviews.length > 0 ? <div className="space-y-4">
                  {reviews.map((review: any) => <div key={review.id} className="border-b border-border pb-4 last:border-0">
                      <div className="flex items-start gap-3">
                        <Avatar>
                          <AvatarImage src={review.explorer?.avatar_url} />
                          <AvatarFallback>
                            {review.explorer?.full_name?.charAt(0) || "U"}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1">
                        ...
                        }).map((_, i) => <Star key={i} className="h-3 w-3 fill-primary text-primary" />)}
                            </div>
                          </div>
                          ...
                          {review.response && <div className="mt-3 pl-4 border-l-2 border-border">
                ...
                </div> : <p className="text-muted-foreground">No reviews yet. Be the first to review!</p>}
            </div>
            ...
            <div className="pb-8">
              ...
              <div className="flex flex-col items-center text-center p-8 border border-border rounded-2xl">
                ...
                <h3 className="text-2xl font-semibold mb-1">
                  {guideProfile?.full_name || "Guide"}
                </h3>
                {guideProfile?.guide_title && <p className="text-muted-foreground mb-2">{guideProfile.guide_title}</p>}
                <p className="text-sm text-muted-foreground mb-4">
                  {guideProfile?.location || ""}
                </p>
                <div className="flex gap-8 mb-6">
                  <div>
                    <p className="text-xl font-semibold">{service.reviews_count}</p>
                ...
                {guideProfile?.bio && <p className="text-sm text-muted-foreground mb-4 max-w-md">
                    {guideProfile.bio}
                  </p>}
                <Button variant="outline" className="w-full max-w-xs" onClick={handleChatWithGuide}>
                  Contact Host
                </Button>
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              <div className="border border-border rounded-2xl p-6 shadow-lg space-y-4">
                ...
                {service.type !== "recorded" && <div className="space-y-3">
                    {/* Date Picker */}
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("w-full justify-start h-14 flex-col items-start", !date && "text-muted-foreground")}>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={date} onSelect={setDate} initialFocus disabled={date => date < new Date()} className="pointer-events-auto" />
                      </PopoverContent>
                    </Popover>

                    {/* Time Slot Selector */}
                    <Select value={timeSlot} onValueChange={setTimeSlot} disabled={!date || slotsLoading}>
                      <SelectTrigger className={cn("w-full justify-start h-14 flex-col items-start", !timeSlot && "text-muted-foreground")}>
